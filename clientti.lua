...
Citizen.CreateThread(function()
	...
	if jengissa then
		TriggerServerEvent('samppialueet:fetchmestat')
	end
end)



RegisterNetEvent('samppialueet:mestat')
AddEventHandler('samppialueet:mestat', function(result)
	if jengissa then
...
RegisterNetEvent('samppialueet:currentlyclaiming')
AddEventHandler('samppialueet:currentlyclaiming', function(mesta)
	alueenperkele = mesta
	holdingup = true
	dfhdfhdfjdfj(mesta)
end)

RegisterNetEvent('samppialueet:menetetty')
AddEventHandler('samppialueet:menetetty', function(kohte)
	SetBlipColour(Areas[kohte].nameofarea,4)
	ESX.ShowNotification('~r~Alueesi on vallattu!!!')
end)

RegisterNetEvent('samppialueet:FIB')
AddEventHandler('samppialueet:FIB', function(kohte)
	SetBlipColour(Areas[kohte].nameofarea,4)
end)

RegisterNetEvent('samppialueet:killblip')
AddEventHandler('samppialueet:killblip', function(kohte)
	SetBlipColour(Areas[kohte].nameofarea,69)
	ESX.ShowNotification('~g~Aluettasi ei saatu vallattua!!!')
	
end)

RegisterNetEvent('samppialueet:setblip')
AddEventHandler('samppialueet:setblip', function(kohte,vari)
	SetBlipColour(Areas[kohte].nameofarea,vari)
	if vari == 59 then
		ESX.ShowNotification("~g~Aluettasi Vallataan!!!")
	else
		ESX.ShowNotification("~g~Kaupungissa vallataan aluetta!")
	end
end)

RegisterNetEvent('samppialueet:toofarlocal')
AddEventHandler('samppialueet:toofarlocal', function()
	holdingup = false
	claimingName = ""
end)

RegisterNetEvent('samppialueet:muille')
AddEventHandler('samppialueet:muille', function(alue)
	ESX.ShowNotification('Jengisi on vallannut alueen')
	SetBlipColour(Areas[alue].nameofarea,69)
end)

RegisterNetEvent('samppialueet:claimcomplete')
AddEventHandler('samppialueet:claimcomplete', function()
	holdingup = false
	alueenperkele = ""
end)

--RegisterNetEvent('samppialueet:valloitusilmoitus') --testi 28.5
--AddEventHandler('samppialueet:valloitusilmoitus', function(notifikaatio)
...
RegisterNetEvent('samppialueet:starttimer')
AddEventHandler('samppialueet:starttimer', function()
	...
	while true do
		...
		if not holdingup then
						...
						if IsControlJustReleased(0, 38) then -- käynnistä ryöstö
							...
							if IsPedArmed(PlayerPedId(), 7) then
								if not jengissa then-- onko asetta								
									...
									TriggerServerEvent('samppialueet:claim', area)
								end
							else
			...
			if Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > Config.MaxDistance then
				TriggerServerEvent('samppialueet:toofar', alueenperkele)
				timer = 0
			end
			...
			if laskuri == Config.Montakoalistetaan then
				TriggerServerEvent('samppialueet:rostoohi', alueenperkele)
				holdingup = false
				laskuri = 0
			end
