...
function juuilmotaomistajillekuvaltaajaleavas(alue)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == "FIB" then
			TriggerClientEvent('samppialueet:FIB', xPlayers[i], alue)
		end
	end
	if sellane[alue].omistaja ~= "" then
		for i=1, #xPlayers, 1 do
			...
			if sellane[alue].omistaja == xPlayer.job.name then
				TriggerClientEvent('samppialueet:killblip', xPlayers[i], alue)
				--TriggerClientEvent("samppialueet:valloitusilmoitus", xPlayers[i], alue)
			end
		end
	end
end
			...
			if result[i].collectattu ~= "" and tonumber(result[i].collectattu) + aikas < os.time() then
				...
				for i=1, #xPlayers, 1 do
					...
					if result[i].omistaja == xPlayer.job.name then
						...
						TriggerClientEvent('samppialueet:menetetty', xPlayers[i], i)
					end
				end
				Citizen.Wait(2500)
				paskavammat()
...
RegisterServerEvent('samppialueet:tieto')
AddEventHandler('samppialueet:tieto', function(alue)	
...
RegisterServerEvent('samppialueet:toofar')
AddEventHandler('samppialueet:toofar', function(alue)
	settaapois(alue)
	TriggerClientEvent('samppialueet:toofarlocal', source)
	local xPlayers = arp.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = arp.GetPlayerFromId(xPlayers[i])
		if xPlayer.job.name == "FIB" then
...
RegisterServerEvent('samppialueet:rostoohi')
AddEventHandler('samppialueet:rostoohi', function(tyo)
	if onkovaltaajatamapelaaja(source,tyo) then
		settaapois(tyo)
		
		TriggerClientEvent('samppialueet:claimcomplete', source)
		...
		for i=1, #xPlayers, 1 do
			local xPlayer = arp.GetPlayerFromId(xPlayers[i])
			if xPlayer.job.name == "FIB" then
				TriggerClientEvent('samppialueet:FIB', xPlayers[i], tyo)
			end
		end
		for i=1, #xPlayers, 1 do
			local xPlayer = arp.GetPlayerFromId(xPlayers[i])
			if tyoukko == xPlayer.job.name then
				TriggerClientEvent('samppialueet:muille', xPlayers[i], tyo)
			end
		end
		if vanhatomistajat ~= "" then
			for i=1, #xPlayers, 1 do
				...
				if vanhatomistajat == xPlayer.job.name then
					TriggerClientEvent('samppialueet:menetetty', xPlayers[i], tyo)
					--TriggerClientEvent("samppialueet:valloitusilmoitus", xPlayers[i], tyo)				
				end
			end
		end
		Citizen.Wait(2500)
		paskavammat()
		Citizen.Wait(2500)
		TriggerClientEvent('samppialueet:mestat', -1, sellane)
	end
end)



RegisterServerEvent('samppialueet:fetchmestat')
AddEventHandler('samppialueet:fetchmestat', function()
	TriggerClientEvent('samppialueet:mestat', source, sellane)
end)
RegisterServerEvent('samppialueet:claim')
AddEventHandler('samppialueet:claim', function(k)
	local source = source
			...
			if sellane[k].omistaja ~= xPlayer.job.name then
				...
				if cops >= Config.TarvittavatPoliisitAloitukseen then
					if omistajiapaikalla >= Config.TarvittavatOmistajatAloitukseen or sellane[k].omistaja == "" then
						...
						TriggerClientEvent("samppialueet:starttimer", source)
						TriggerClientEvent("samppialueet:currentlyclaiming", source, k)
						for i=1, #xPlayers, 1 do
							local xPlayer = arp.GetPlayerFromId(xPlayers[i])
							if xPlayer.job.name == "FIB" then
								TriggerClientEvent('samppialueet:setblip', xPlayers[i], k,5)
							end
						end
						if sellane[k].omistaja ~= "" then
							for i=1, #xPlayers, 1 do
								local xPlayer = arp.GetPlayerFromId(xPlayers[i])
								if sellane[k].omistaja == xPlayer.job.name then
									--TriggerClientEvent("samppialueet:valloitusilmoitus", xPlayers[i], k)
									TriggerClientEvent('samppialueet:setblip', xPlayers[i], k,59)
								end
							end
						end
					else
